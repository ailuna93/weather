...
function App() {
    ...
    const getPhoto = (lon, lat) => {
        axios(`https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=1452866c8cea54acd0075022ef573a07&lat=${lat}
            &lon=${lon}&accuracy=1tags=square&sort=relevance&extras=url_l&format=json`)
                ...
                <section className="weather">
                    ...
                    >
                      ...
                      °F
                                ...
                                <p className="weather-time">{toDate(Date.now())}</p>

                                <p className="weather-humidity">
                                    Влажность:{" "}
                                    <span className="weather__humidity-count">{weather.main.humidity} %
                  </span>
                                </p>
                                <button
                                    type="button"
                                    className="weather__5day"
                                    onClick={getWeatherForFiveDays}
                                >
                            ...
                            return (
                                <div className="weather-five__row" key={item}>
                                    <div className="weather-five__main">
                                        <p className="weather-five__main-date">{item}</p>
                                    </div>
                                        ...
                                        {weatherFive.list.map((el) => {
                                            if (el.dt_txt.includes(item))
                                                return (
                                                    <li
                                                        key={el.dt__txt}
                                                        className="weather-five__menu-list">
                                                        <p>{el.dt__txt}</p>
                                                        <img
